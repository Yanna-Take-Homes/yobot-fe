...
const PastChats = () => {
    ...
    return (
        <PastChatsWrapper>
            ...
            {chats && chats.length ? chats.map((chat, idx) => (<p key={idx}>{chat["lesson_name"]}</p>)) :
                <p>You've not done any lessons yet!</p>}
            <Button size={"large"} type={"link"}><Link to={"/chat"}>Start New Chat</Link></Button>
        </PastChatsWrapper>
    );
}
